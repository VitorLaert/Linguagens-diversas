/*******************************************************************************
* Master Kit - 26. Projeto Garra Robótica
...
void loop() {
  ...
  if (leitura_vry >= 550) {
    abrir = true; // Seta a flag caso o movimento do joystick indique uma tendência de abertura
  }
  else if (leitura_vry <= 490) {
    fechar = true; // Seta a flag caso o movimento do joystick indique uma tendência de fechamento
  }

  // Se a flag de abrir estiver ativada
  if (abrir) {
    leitura_vry = analogRead(VRY);
    ...
    if (leitura_sw == HIGH) { // Verifica o botão
      ...
      if (leitura_vry > posi_atual) { // Se a posição da garra for maior que a posição anterior
        posi_atual = leitura_vry; // Atualiza a variável de posição
        leitura_vry = map(leitura_vry, 0, 1023, CLOSE, OPEN);
        servo_motor.write(leitura_vry);
      }
    }
    // Atualiza a flag e a posição da garra se o botão for pressionado
    else {
      abrir = false; // Resetar a flag
      posi_atual = 520; // Resetar a posição
    }
  }

  // Se a flag de fechar estiver ativada
  if (fechar) {
    leitura_vry = analogRead(VRY);
    leitura_sw = digitalRead(SW);
    ...
    if (leitura_sw == HIGH) { // Verifica o botão
      // Verifica a posição da garra
      if (leitura_vry < posi_atual) { // Se a posição da garra for menor que a posição anterior
        posi_atual = leitura_vry; // Atualiza a variável de posição
        leitura_vry = map(leitura_vry, 0, 1023, CLOSE, OPEN);
        servo_motor.write(leitura_vry);
      }
